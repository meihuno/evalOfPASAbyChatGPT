...
class ChatConversation(object):
    """ChatGPTにプロンプトで対話する
    """
    def __init__(self, model_name='gpt-3.5-turbo', base_dir='./result/dev4', async_flag=False, prompt_type='gogo', sample_limit=1000, process_env='local', skip_process_flag=False):
    ...
    def _ret_batch_dict(self, target_id_list, prompt_files):
        if self.prompt_type == 'extra':
            return self._ret_extra_batch_dict(target_id_list, prompt_files)
        else:
    ...
    def _ret_gogo_batch_dict(self, target_id_list, prompt_files):
        ...
        for id1 in target_id_list:
            ...
            for exp_name, prompt_dict1 in prompt_dict.items():
                ...
                if exp_name == 'order-sequence' or exp_name == 'zero-shot-simple' or exp_name =='zero-shot-modify-knp' or exp_name == 'few-shot-simple' or exp_name == 'few-shot-simple-semantic-search' or exp_name == 'summary':
...
def main():
    ...
    ok_target_id_list = ok_target_id_list + exist_5_id_list
